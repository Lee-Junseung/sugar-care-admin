...
> 사용자의 혈당·식단·알레르기·목표 데이터를 관리자가 한눈에 모니터링하고, 신고 내역을 관리할 수 있는 화면을 구현함.
...
- 배포 환경: 별도 백엔드 API 서버 연동 (`VITE_API_URL` 환경변수로 지정)
...
`App.jsx`의 메뉴 구조 기준으로 구현된 화면입니다.
...
### 데모 버전 (demo 브랜치)
백엔드 서버 없이도 화면을 확인할 수 있도록, mock 데이터로 대체한 데모 버전을 별도 브랜치로 관리하고 있습니다.
...
배포 링크: (Vercel/Netlify 배포 후 링크 추가 예정)

> 로그인 정보는 현재 코드 내 데모용 계정으로 동작합니다. 실제 배포 시에는 백엔드 인증 방식으로 전환이 필요합니다.
